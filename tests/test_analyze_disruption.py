...
class TestAnalyzeDisruption:
    ...
    '''
    def test_missing_trip_id_returns_422(client, valid_request_data):
        data = copy.deepcopy(valid_request_data)
        data["trip_id"] = ""
        response = client.post("/analyze_disruption", json=data)
        assert response.status_code == 422

    def test_negative_delay_returns_422(client, valid_request_data):
        data = copy.deepcopy(valid_request_data)
        data["flights"][0]["delay_mins"] = -10
        response = client.post("/analyze_disruption", json=data)
        assert response.status_code == 422

    def test_empty_weather_returns_422(client, valid_request_data):
        data = copy.deepcopy(valid_request_data)
        data["weather"] = {}
        response = client.post("/analyze_disruption", json=data)
        assert response.status_code == 422
